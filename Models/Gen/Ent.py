...
class FeatureEncoder(nn.Module):
  def __init__(self, state_dim, latent_dim):
    super().__init__()
    self.fc = nn.Sequential(nn.Linear(state_dim, 64),
                            nn.ReLU(),
...
class ForwardDynamicsModel(nn.Module):
  def __init__(self, latent_dim, action_dim):
    super().__init__()
    self.fc = nn.Sequential(nn.Linear(latent_dim + action_dim, 64),
                            nn.ReLU(),
                            nn.Linear(64, latent_dim))
...
class DiffusionModel(nn.Module):
  def __init__(self, state_dim, action_dim, latent_dim=64, timesteps=4):
    ...
    self.net = nn.Sequential(nn.Linear(state_dim + action_dim + 1, 128),
                             nn.ReLU(),
...
class GaussianPolicy(nn.Module):
  def __init__(self, state_dim, action_dim, hidden_dim=64):
    super().__init__()
    self.fc_mean = nn.Sequential(nn.Linear(state_dim, hidden_dim),
                                 nn.ReLU(),
...
def main():
  ...
  for ep in range(episodes):
    ...
    if len(real_buffer) > batch_size and len(syn_buffer) > batch_size:
      train_agent(policy, q_net, optimizer_q, real_buffer, syn_buffer, batch_size,
                  gamma=0.99, entropy_coef=entropy_coef)
