...
def process_logs(logs_folder=".logs"):
    ...
    for model_name in os.listdir(logs_folder):
        ...
        studies = load_and_copy_studies(model_folder_path, result_path)
        if not studies:
            print(f"No valid Optuna studies found for model: {model_name}")
            continue

        for idx, study in enumerate(studies):
            save_optuna_plots(study, f"{model_name}_{idx}", result_path)
            plot_custom_metrics_best_trial(study, result_path, f"{model_name}_{idx}")
